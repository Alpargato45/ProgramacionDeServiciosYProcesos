...
public class GestorEmail {
    ...
    public void enviarMensajeConAdjuntos(String emisor, String destinatario, String asunto, String textoMensaje,
            String direccionEmail, String password, List<String> pathsFicheros)
            throws AddressException, MessagingException, IOException {

        setPropiedadesServidorSMTP();
        Message mensaje = crearMensajeConAdjuntos(emisor, destinatario, asunto, textoMensaje, pathsFicheros);
        Transport t = conectarServidorSMTP(direccionEmail, password);
        t.sendMessage(mensaje, mensaje.getAllRecipients());
        t.close();
    }

    private Message crearMensajeConAdjuntos(String emisor, String destinatario, String asunto, String textoMensaje,
            List<String> pathsFicheros) throws MessagingException, AddressException, IOException {
        Message mensaje = crearNucleoMensaje(emisor, destinatario, asunto);
        BodyPart bodyPart = new MimeBodyPart();
        bodyPart.setText(textoMensaje);
        Multipart multipart = new MimeMultipart();
        multipart.addBodyPart(bodyPart);
        for (String pathFichero : pathsFicheros) {
            MimeBodyPart adjuntoParte = new MimeBodyPart();
            DataSource fuente = new FileDataSource(pathFichero);
            adjuntoParte.setDataHandler(new DataHandler(fuente));
            adjuntoParte.setFileName(fuente.getName());
